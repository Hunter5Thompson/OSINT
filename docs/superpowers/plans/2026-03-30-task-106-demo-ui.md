...
git add services/backend/app/models/events.py services/backend/app/routers/graph.py \
    services/backend/tests/unit/test_graph_router.py
git commit -m "feat(backend): add /graph/events/geo endpoint with location resolution
...
- [ ] **Step 4: Update EntityClickHandler — add event billboard guard**
