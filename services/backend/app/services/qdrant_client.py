...
``QdrantSchemaMismatchError`` before the first search or write, preventing silent
...
__all__ = ["get_qdrant_client", "QdrantSchemaMismatchError"]
...
async def get_qdrant_client() -> AsyncQdrantClient:
    ...
    matches the active runtime mode.  Raises ``QdrantSchemaMismatchError`` if the
