...
- Embedding-Server returned Vektoren mit korrekter Dimension (512)
...
def validate_cypher_readonly(cypher: str) -> bool:
    ...
    write_keywords = r'\b(CREATE|MERGE|DELETE|DETACH|SET|REMOVE|DROP|CALL\s+\{)\b'
    return not bool(re.search(write_keywords, cypher, re.IGNORECASE))
...
point = PointStruct(
    id=uuid4().hex,
    vector={
        "dense": Document(text=chunk_text, model="Qwen/Qwen3-Embedding-0.6B"),
